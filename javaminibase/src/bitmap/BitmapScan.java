 ...
 * of the heapfile.
 */
public class BitmapScan implements GlobalConst {

    /** The heapfile we are using. */
    private BitMapFile _hf;
    ...
    private BMPage datapage = new BMPage();

    /** record ID of the current record (from the current data page)(will be a combination of page id
     * and position )*/
     ...
     *
     * @param bmf A bitmapfile object
     */
    public BitmapScan(BitMapFile bmf)
    ...
    public Boolean getNext(RID rid)
        ...
        try {
            bit = datapage.getRecord(rid);
        } catch (Exception e) {
            //    System.err.println("SCAN: Error in Scan" + e);
            e.printStackTrace();
        }

        userrid.pageNo = datapage.getCurPage();
        userrid.slotNo += 1;
        if (userrid.slotNo >= datapage.getRecordCnt()) {
     ...
     */
    private void init(BitMapFile hf)
            throws InvalidTupleSizeException,
            IOException {
        _hf = hf;
    ...
    private void pinPage(PageId pageno, Page page, boolean emptyPage)
        ...
        try {
            ...
            throw new BMBufMgrException(e, "Scan.java: pinPage() failed");
        }
